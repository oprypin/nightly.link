...
macro cached_array(f, norm = {} of String => String)
  ...
  @@cache_{{f.name}} = CleanedMemoryCache({ {{f.args.map(&.restriction).splat}} }, Array({{typ}})).new

  {% for block in [true, false] %}
    def {% if f.receiver %}{{f.receiver}}.{% end %}{{f.name}}({{f.args.splat}}, expires_in : Time::Span) : Array({{typ}})
      {% for arg in f.args %}
        {% if arg.restriction.id == "DowncaseString".id %}
          {{arg.internal_name}} = {{arg.internal_name}}.downcase
        {% end %}
      {% end %}
      @@cache_{{f.name}}.fetch({ {{f.args.map(&.internal_name).splat}} }, expires_in: expires_in) do
